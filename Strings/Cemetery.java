...
public class Cemetery
{
   ...
   */
   ...
   {
       String name="",date="",age="";
       
        for(int i =0; i<entry.length();i++)
            if(Character.isDigit(entry.charAt(i))||entry.substring(i,i+2).equals("xx")){
                name=entry.substring(0,i).trim();
                date = entry.substring(i,i+11);
                int j =0;
                String digits="1234567890.wd";
                while(digits.contains(entry.substring(i+12+j,i+13+j))){
                age +=entry.charAt(i+12+j);
                j++;}
                age=age.trim();
                break;
            }
   ...
   */
   ...
   if (arr.length==0)
    ...
    int maxIndex=0;
    for(int i=0;i<arr.length;i++)
        if(arr[i].getAge()>arr[maxIndex].getAge())
            maxIndex=i;
    return maxIndex;
   }        
} 
...
class Person
{
   ...
   */
   ...
   if(a.contains("d"))
    ...
    if(a.contains("w"))
       return Double.parseDouble(df.format(Double.parseDouble(a.substring(0,a.indexOf("w")))/52));
