...
public class AreaFill
{
   ...
   public static char[][] read(String filename)
   {
      ...
      String[] dims=infile.nextLine().split(" ");
      char[][] arr=new char[Integer.parseInt(dims[0])][Integer.parseInt(dims[1])];
      int row =0;
      while(infile.hasNextLine()){
          arr[row]=infile.nextLine().toCharArray();
          row++;
      }
     return arr;
   }
   ...
   public static int fillAndCount(char[][] g, int r, int c, char ch)
   {
      if(r>=0&&r<g.length&&c>=0&&c<g[0].length)
        if(g[r][c]==ch){
            g[r][c]='*';
            if((r==g.length-1||g[r+1][c]!=ch)&&(r==0||g[r-1][c]!=ch)&&(c==g[0].length-1||g[r][c+1]!=ch)&&(c==0||g[r][c-1]!=ch))
                return 1;//dead end
            else return(1+fillAndCount(g,r+1,c,ch)+fillAndCount(g,r,c+1,ch)+fillAndCount(g,r-1,c,ch)+fillAndCount(g,r,c-1,ch));
        }
        return 0;
   }
}
