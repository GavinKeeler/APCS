 ...
 */ 
   ...
   {
      if(r<0||c<0||r>=maze.length||c>=maze[0].length||maze[r][c]=='W'||maze[r][c]=='o') return 999;
      if(maze[r][c]=='E') return 0;
      if(maze[r][c]=='.') maze[r][c]='o';
      int U=findShortestLengthPath(r-1,c);
      int R=findShortestLengthPath(r,c+1);
      int D=findShortestLengthPath(r+1,c);
      int L=findShortestLengthPath(r,c-1);
      if(maze[r][c]=='o') maze[r][c]='.';
      return(1+Math.min(U,Math.min(D,Math.min(L,R))));
   }  
 ...
 */
   public String findShortestPath(int r, int c)
   {
      ...
      int L=findShortestLengthPath(r,c-1);
      if(maze[r][c]=='o') maze[r][c]='.';
      int minPath=Math.min(U,Math.min(D,Math.min(L,R)));
      if(U==minPath)
          return ("("+r+","+c+"),"+(minPath+1)+"),"+findShortestPath(r-1,c));
      else if(R==minPath)
          return ("("+r+","+c+"),"+(minPath+1)+"),"+findShortestPath(r,c+1));
      else if(D==minPath)
          return ("("+r+","+c+"),"+(minPath+1)+"),"+findShortestPath(r+1,c));
      else if(L==minPath)
          return ("("+r+","+c+"),"+(minPath+1)+")"+findShortestPath(r,c-1));
   ...
   {
        if (strPath.length()==0) return 999;
        return strPath.replace("(","").replace(")","").split(",").length/3-1;
   } 
   ...
   */
   public void markPath(String strPath)
   {
      if(strPath.equals(""))
         return;
      String[] arr=strPath.replace("(","").replace(")","").split(",");
      for(int i =3;i<arr.length-3;i+=3){
          maze[Integer.parseInt(arr[i])][Integer.parseInt(arr[i+1])]='*';
      } 
   }
}
