...
class Searches
{
	...
	{
	    binaryCount++;
		if(start>end||Arrays.binarySearch(array,target)<0)
			return -1;
		else if(array[start+(end-start)/2].equals(target))
			return start+(end-start)/2;
		else if(array[start+(end-start)/2].compareTo(target)<0)
			return binaryhelper(array,target,start+(end-start)/2,end);
		else if(array[start+(end-start)/2].compareTo(target)>0)
		return binaryhelper(array,target,start,start+(end-start)/2);
		return -1;
	}
}
